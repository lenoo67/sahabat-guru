...
const ApalGenerator = () => {
  ...
  const handleExportWord = () => {
    ...
    let html = `
      ...
      <head><meta charset="utf-8"><title>Sahabat Guru - ${globalConfig.subject}</title>
      <style>
  ...
  <div className="fixed inset-0 z-[999] flex items-start justify-center p-4 sm:items-center">
    ...
    {/* Modal Card */}
    <div className="relative bg-white w-full max-w-2xl mx-4 rounded-xl shadow-2xl border border-stone-200 overflow-hidden max-h-[90vh] flex flex-col">
      <div className="bg-emerald-800 px-6 py-4 flex items-center justify-between">
        <div className="text-white">
          ...
          <div className="text-xs text-emerald-100 mt-1">
            Berlaku hanya untuk bagian soal yang sedang ditambahkan.
          </div>
        </div>
        <button
          onClick={() => setIsCustomInstrModalOpen(false)}
      ...
      <div className="p-6 space-y-4 overflow-y-auto flex-1">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-stone-700">
          <div className="font-bold text-stone-800 mb-1">Panduan Instruksi</div>
          <div className="text-xs leading-relaxed text-stone-700">
            Gunakan kolom ini untuk memberikan konteks tambahan agar soal yang dihasilkan lebih spesifik.
            Instruksi ini hanya berlaku untuk bagian yang sedang Anda buat.
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full border border-stone-300 bg-white text-stone-900 placeholder:text-stone-400 rounded-lg p-3 text-sm h-28 resize-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
            placeholder='Contoh: "Buat soal dalam Bahasa Inggris" atau "Gunakan konteks kearifan lokal"'
          />
        </div>
      </div>
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row gap-2 justify-end">
        <button
          ...
          className="px-4 py-2 rounded-lg border border-stone-300 bg-white hover:bg-stone-100 text-stone-700 text-sm font-bold"
        >
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg bg-emerald-800 hover:bg-emerald-900 text-white text-sm font-bold flex items-center justify-center gap-2 transition-colors"
        >
      ...
      {imageProgress.active && (
          <div className="fixed bottom-4 right-4 z-50 animate-in slide-in-from-bottom-5 duration-300">
              <div className="bg-white rounded-xl shadow-xl border border-emerald-200 p-4 w-72 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                          <Loader2 size={16} className="text-emerald-600 animate-spin" />
                          <span className="text-sm font-bold text-stone-800">Memproses Gambar...</span>
                      </div>
      ...
      {/* --- SIDEBAR --- */}
      {showSidebar && (
      <aside className={`w-56 bg-white border-r border-stone-200 fixed h-full z-40 flex flex-col shadow-sm transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <div className="h-16 flex items-center justify-between px-6 bg-emerald-800 border-b border-emerald-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white leading-none tracking-tight uppercase">Sahabat Guru</h1>
              <p className="text-[9px] text-emerald-100 font-medium uppercase tracking-wider mt-0.5">Teman Setia Pendidik Indonesia</p>
            </div>
          </div>
        ...
        <div className="bg-emerald-50 px-6 py-2 border-b border-emerald-100 flex items-center justify-between">
           <span className="text-[10px] font-bold text-emerald-800">Versi 2.0</span>
           <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span className="text-[10px] font-medium text-stone-600">Terhubung</span>
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-1">
          ...
          <div className="px-4 py-2">
             ...
             <div className="space-y-0.5">
                ...
                <button 
                   ...
                   className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition-colors border-l-4 ${currentView === 'identity' ? 'bg-blue-50 border-blue-600 text-blue-800' : 'border-transparent text-slate-600 hover:bg-gray-50 hover:text-slate-900'}`}
                >
                    <User size={16} className={`${currentView === 'identity' ? 'text-blue-700' : 'text-slate-400'}`} />
                    Identitas Soal
                </button>
                ...
                <div className={`${!isIdentityComplete ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    <button 
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 text-sm font-medium transition-colors border-l-4 ${currentView === 'technical' ? 'bg-emerald-50 border-emerald-600 text-emerald-800' : 'border-transparent text-stone-600 hover:bg-stone-50'}`}
                        disabled={!isIdentityComplete}
                    >
                        <div className="flex items-center gap-3">
                            <Settings size={16} className={`${currentView === 'technical' ? 'text-emerald-700' : 'text-stone-400'}`} />
                            <div className="flex items-center gap-2">
                                <span>Teknis Soal</span>
                                {!isIdentityComplete && <Lock size={12} className="text-stone-400" />}
                            </div>
                        </div>
                        {isTechnicalMenuOpen ? <ChevronDown size={14} className="text-stone-400" /> : <ChevronRight size={14} className="text-stone-400" />}
                    </button>
                    ...
                    {isTechnicalMenuOpen && (
                        <div className="pl-9 space-y-1 mt-1 mb-2">
                            <button 
                                onClick={() => navigateTo('technical')}
                                disabled={!isIdentityComplete}
                                className={`w-full flex items-center gap-2 py-1.5 text-xs transition-colors text-left ${currentView === 'technical' ? 'text-emerald-700 font-semibold' : 'text-stone-500 hover:text-stone-800'}`}
                            >
                                <ChevronRight size={12} className={currentView === 'technical' ? 'text-emerald-700' : 'text-stone-400'} />
                                <span>Tambah Bagian</span>
                            </button>
                            <button 
                                onClick={() => navigateTo('technical')}
                                disabled={!isIdentityComplete}
                                className={`w-full flex items-center gap-2 py-1.5 text-xs transition-colors text-left ${currentView === 'technical' ? 'text-blue-700 font-semibold' : 'text-slate-500 hover:text-slate-800'}`}
                            >
                                <ChevronRight size={12} className={currentView === 'technical' ? 'text-blue-700' : 'text-slate-400'} />
          ...
          <div className="px-4 py-2">
            ...
            <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition-colors border-l-4 ${!isIdentityComplete || !generatedData ? 'opacity-50 cursor-not-allowed border-transparent text-stone-400' : currentView === 'quiz' ? 'bg-emerald-50 border-emerald-600 text-emerald-800' : 'border-transparent text-stone-600 hover:bg-stone-50 hover:text-stone-900'}`}
            >
                <Gamepad2 size={16} className={`${currentView === 'quiz' ? 'text-emerald-700' : 'text-stone-400'}`} />
                <div className="flex items-center gap-2">
                    <span>Mode Quiz</span>
                    {(!isIdentityComplete || !generatedData) && <Lock size={12} className="text-stone-400" />}
                </div>
            </button>
          </div>
          ...
          <div className="px-4 py-2">
            ...
            <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition-colors border-l-4 ${currentView === 'tutorial' ? 'bg-emerald-50 border-emerald-600 text-emerald-800' : 'border-transparent text-stone-600 hover:bg-stone-50 hover:text-stone-900'}`}
            >
                <HelpCircle size={16} className={`${currentView === 'tutorial' ? 'text-emerald-700' : 'text-stone-400'}`} />
                <span>Tutorial</span>
            </button>
            <button 
                onClick={() => navigateTo('legal')}
                className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium transition-colors border-l-4 ${currentView === 'legal' ? 'bg-emerald-50 border-emerald-600 text-emerald-800' : 'border-transparent text-stone-600 hover:bg-stone-50 hover:text-stone-900'}`}
            >
                <ShieldCheck size={16} className={`${currentView === 'legal' ? 'text-emerald-700' : 'text-stone-400'}`} />
                <span>Ketentuan</span>
            </button>
          </div>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
           <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-sm bg-blue-800 flex items-center justify-center text-white font-bold text-xs uppercase relative">
                  G
                  <div className="absolute -top-1.5 -right-1.5 bg-yellow-400 rounded-full p-0.5 border-2 border-white">
                      <Crown size={8} className="text-blue-900" fill="currentColor" />
                  </div>
              </div>
              <div className="flex-1 overflow-hidden">
                 ...
                 <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="px-1.5 py-0.5 bg-yellow-100 text-yellow-700 text-[8px] font-bold uppercase tracking-wider border border-yellow-200 rounded-sm">Premium</span>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className={`flex-1 relative flex flex-col min-h-screen bg-stone-100 ${showSidebar ? 'md:ml-56' : ''}`}>
        ...
        <header className="bg-emerald-800 border-b border-emerald-900 p-4 md:hidden sticky top-0 z-30 flex items-center justify-between text-white shadow-md">
           <div className="flex items-center gap-2">
              ...
              <span className="font-bold tracking-tight">Sahabat Guru Mobile</span>
           </div>
        ...
        <main id="main-content-scroll" className={`flex-1 overflow-y-auto w-full ${currentView !== 'welcome' ? (isPlayMode ? 'p-0' : 'pb-24 md:pb-32 p-6 md:p-10 max-w-6xl mx-auto') : 'p-0'}`}>
          ...
          {currentView === 'welcome' && (
              <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-950 flex flex-col justify-center items-center p-6 text-white text-center">
                  <div className="max-w-3xl w-full animate-in fade-in zoom-in duration-700">
                      ...
                      <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 drop-shadow-sm">
                          Selamat Datang di <span className="text-amber-300">Sahabat Guru</span>
                      </h1>
                      <p className="text-lg md:text-xl text-emerald-100 mb-10 max-w-2xl mx-auto leading-relaxed">
                          Asisten Cerdas Pendidik Indonesia. Solusi inovatif untuk menyusun naskah ujian dengan cepat, akurat, dan menyenangkan.
                      </p>
                      ...
                      <button 
                          onClick={() => navigateTo('legal')}
                          className="bg-amber-400 hover:bg-amber-300 text-emerald-900 font-extrabold py-4 px-10 rounded-full shadow-xl shadow-amber-400/20 transform hover:-translate-y-1 transition-all flex items-center gap-3 mx-auto text-lg"
                      >
                          Mulai Sekarang <ArrowRight size={24} />
                      </button>

                      <button 
                          onClick={() => alert("Pendaftaran pengguna baru dibatasi untuk institusi terdaftar.")}
          ...
          <div className="mb-6 pb-4 border-b border-stone-200">
            ...
            {(currentView === 'technical' || currentView === 'tutorial') && (
                <button 
                    onClick={() => navigateTo('identity')} 
                    className="flex items-center gap-1.5 text-xs font-bold text-stone-500 hover:text-emerald-800 mb-2 transition-colors uppercase tracking-wide group"
                >
            ...
            <h2 className="text-2xl font-bold text-stone-800 uppercase tracking-tight">
            ...
            <p className="text-stone-500 text-sm mt-1">
                ...
                {currentView === 'tutorial' && 'Pelajari langkah-langkah mudah menyusun soal otomatis dengan Sahabat Guru.'}
             ...
             <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-4xl mx-auto">
                 <div className="bg-white p-8 rounded-xl shadow-sm border border-stone-200">
                     ...
                     <div className="flex items-center gap-3 mb-6 pb-4 border-b border-stone-200">
                         ...
                         <div>
                             <h3 className="text-xl font-bold text-stone-800 uppercase tracking-tight">Penafian & Batasan Tanggung Jawab</h3>
                             <p className="text-sm text-stone-500">Terakhir diperbarui: 27 Desember 2025</p>
                         </div>
                     </div>

                     <div className="space-y-6 text-stone-700 text-justify leading-relaxed text-sm font-serif">
                         
                         <p>
                             Dengan menggunakan aplikasi <strong>Sahabat Guru</strong>, Anda (selanjutnya disebut sebagai "Pengguna") dianggap telah membaca, memahami, dan menyetujui seluruh syarat dan ketentuan yang tercantum di bawah ini.
                         </p>

                         <div className="bg-stone-50 p-4 border-l-4 border-emerald-800 rounded-r-lg">
                         ...
                         <div className="border-t border-gray-200 pt-6 mt-8 text-center">
                             ...
                             <button
                                 onClick={() => navigateTo('identity')}
                                 className="bg-blue-800 hover:bg-blue-900 text-white font-bold py-3 px-12 rounded-full shadow-lg transition-transform transform hover:-translate-y-0.5 flex items-center gap-2 mx-auto"
                             >
             ...
             <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl">
                 ...
                 <div className="flex gap-6">
                    <div className="flex-shrink-0">
                       <div className="w-12 h-12 bg-blue-800 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border-4 border-blue-50">1</div>
                    </div>
                    <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex-1 relative">
                       <div className="absolute top-4 right-4 text-blue-100"><User size={48} /></div>
                 ...
                 <div className="flex gap-6">
                    <div className="flex-shrink-0">
                       <div className="w-12 h-12 bg-blue-800 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border-4 border-blue-50">2</div>
                    </div>
                    <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex-1 relative">
                       <div className="absolute top-4 right-4 text-blue-100"><Settings size={48} /></div>
                 ...
                 <div className="flex gap-6">
                    <div className="flex-shrink-0">
                       <div className="w-12 h-12 bg-blue-800 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border-4 border-blue-50">3</div>
                    </div>
                    <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex-1 relative">
                       <div className="absolute top-4 right-4 text-blue-100"><Wand2 size={48} /></div>
                 ...
                 <div className="flex gap-6">
                    <div className="flex-shrink-0">
                       <div className="w-12 h-12 bg-blue-800 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border-4 border-blue-50">4</div>
                    </div>
                    <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex-1 relative">
                       <div className="absolute top-4 right-4 text-blue-100"><FileEdit size={48} /></div>
                 ...
                 <div className="flex justify-center pt-8 pb-12">
                     <button 
                        onClick={() => navigateTo('identity')}
                        className="bg-blue-800 hover:bg-blue-900 text-white font-bold py-3 px-8 rounded-full shadow-lg transform hover:-translate-y-1 transition-all flex items-center gap-3"
                     >
          ...
          {currentView === 'identity' && (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="bg-white shadow-sm border border-stone-200 rounded-xl overflow-hidden mb-6">
                      <div className="bg-stone-50 px-6 py-3 border-b border-stone-200 flex items-center gap-2">
                          ...
                          <h3 className="text-sm font-bold text-stone-700 uppercase">Formulir Identitas</h3>
                      </div>
                      ...
                      <div className="p-6 space-y-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div id="input-teacherName">
                                  <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Nama Guru / Penyusun</label>
                                  <input type="text" className={`w-full border border-stone-300 bg-white text-stone-900 placeholder:text-stone-400 rounded-lg p-2 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all ${identityErrors.teacherName ? 'border-red-500 bg-red-50' : 'border-stone-300'}`} 
                              ...
                              <div id="input-institution">
                                  <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Nama Satuan Pendidikan</label>
                                  <input type="text" className={`w-full border border-stone-300 bg-white text-stone-900 placeholder:text-stone-400 rounded-lg p-2 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all ${identityErrors.institution ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                          ...
                          <div className="bg-emerald-50/50 p-5 rounded-xl border border-emerald-100">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                  <div id="input-educationLevel">
                                      <label className="block text-xs font-bold text-stone-700 mb-1.5">1. Jenjang Pendidikan</label>
                                      <select className={`w-full border rounded-lg p-2 text-sm bg-white text-stone-900 focus:ring-1 focus:ring-emerald-500 outline-none ${identityErrors.educationLevel ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                                  ...
                                  <div id="input-phase">
                                      <label className={`block text-xs font-bold mb-1.5 ${!globalConfig.educationLevel ? 'text-stone-400' : 'text-stone-700'}`}>
                                          2. {globalConfig.educationLevel === 'Kesetaraan' ? 'Paket' : 'Fase Pembelajaran'}
                                      </label>
                                      <select className={`w-full border rounded-lg p-2 text-sm bg-white text-stone-900 focus:ring-1 focus:ring-emerald-500 outline-none disabled:bg-stone-100 disabled:text-stone-400 ${identityErrors.phase ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                                  ...
                                  <div id="input-grade">
                                      <label className={`block text-xs font-bold mb-1.5 ${!globalConfig.phase ? 'text-stone-400' : 'text-stone-700'}`}>
                                          3. Kelas
                                      </label>
                                      <select className={`w-full border rounded-lg p-2 text-sm bg-white text-stone-900 focus:ring-1 focus:ring-emerald-500 outline-none disabled:bg-stone-100 disabled:text-stone-400 ${identityErrors.grade ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                          ...
                          <div className="grid grid-cols-1 gap-6">
                              <div id="input-subject">
                                  <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Mata Pelajaran</label>
                                  <input type="text" className={`w-full border border-stone-300 bg-white text-stone-900 placeholder:text-stone-400 rounded-lg p-2 text-sm focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none ${identityErrors.subject ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                              ...
                              <div id="input-topic">
                                  <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Topik / Lingkup Materi</label>
                                  <textarea className={`w-full border border-stone-300 bg-white text-stone-900 placeholder:text-stone-400 rounded-lg p-2 text-sm h-24 resize-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none ${identityErrors.topic ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                  ...
                  <div className="flex justify-end pt-4 border-t border-gray-200">
                      <button 
                        ...
                        className="bg-emerald-800 hover:bg-emerald-900 text-white font-bold py-2.5 px-6 rounded-lg shadow-sm flex items-center gap-2 transition-colors text-sm"
                      >
          ...
          {currentView === 'technical' && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="lg:col-span-5 space-y-6">
                      <div className="bg-white shadow-sm border border-stone-200 rounded-xl overflow-hidden">
                          <div className="bg-emerald-50 px-5 py-3 border-b border-emerald-100 flex items-center gap-2">
                             ...
                             <h3 className="text-sm font-bold text-stone-700 uppercase">Input Bagian Soal</h3>
                          </div>
                          ...
                          <div className="p-5 space-y-5">
                              ...
                              <div className="space-y-4" id="input-questionType">
                                  <div>
                                      <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Bentuk Soal</label>
                                      <select className={`w-full border rounded-lg p-2 text-sm bg-white text-stone-900 focus:ring-1 focus:ring-emerald-500 outline-none ${formErrors.questionType ? 'border-red-500 bg-red-50' : 'border-stone-300'}`}
                                  ...
                                  {(currentSection.questionType.includes('Pilihan Ganda')) && (
                                      <div>
                                          <label className="block text-xs font-bold text-stone-600 mb-1.5 uppercase">Jumlah Opsi</label>
                                          <div className="flex border border-stone-300 rounded-lg overflow-hidden bg-white">
                                              {['3', '4', '5'].map(opt => (
                                                <button
                                                    ...
                                                    className={`flex-1 py-1.5 text-xs font-bold border-r last:border-r-0 border-stone-200 transition-colors ${
                                  ...
                                  <div 
                                      className={`p-3 rounded-lg border cursor-pointer flex items-start justify-between transition-colors ${currentSection.withImage ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-stone-300 hover:bg-stone-50'}`}
                                      onClick={() => setCurrentSection({...currentSection, withImage: !currentSection.withImage})}
                                  >
                                      <div className="flex items-start gap-3">
                                          <ImageIcon size={16} className={`mt-0.5 ${currentSection.withImage ? 'text-emerald-700' : 'text-stone-400'}`} />
                                          <div>
                                              <span className="text-xs font-bold text-stone-700 uppercase block">tambah gambar di naskah soal</span>
                                              <p className="text-[10px] text-stone-500 mt-1 font-normal leading-tight">Gambar bisa saja kurang akurat, mohon periksa ulang hasil gambarnya.</p>
                                          </div>
                                      </div>
                                      <div className={`mt-0.5 w-4 h-4 border border-stone-400 rounded-sm flex items-center justify-center flex-shrink-0 ${currentSection.withImage ? 'bg-emerald-700 border-emerald-700' : 'bg-white'}`}>
                              ...
                              <div id="input-difficulties">
                                  <label className="block text-xs font-bold text-stone-600 mb-2 uppercase">Tingkat Kesulitan</label>
                                  ...
                                  <div className={`space-y-2 p-3 rounded-lg border ${formErrors.difficulties ? 'bg-red-50 border-red-300' : 'bg-stone-50 border-stone-200'}`}>
                                          ...
                                          <div key={level} className="flex items-center justify-between">
                                              <label className="flex items-center gap-2 cursor-pointer text-sm text-stone-700">
                                                  <input type="checkbox" checked={currentSection.difficulties[level]} onChange={() => handleDiffCheck(level)} 
                                                      className="w-4 h-4 rounded-sm border-stone-300 text-emerald-700 focus:ring-emerald-500" />
                                                  <span className="font-medium">{level}</span>
                                              </label>
                                              {currentSection.difficulties[level] && (
                                                  <input type="number" className="w-16 p-1 text-sm bg-white text-stone-900 border border-stone-300 rounded-lg text-center focus:ring-1 focus:ring-emerald-500 outline-none" min="1"
                                  ...
                                  <div className="mt-2 text-right">
                                      <span className="text-xs font-bold text-stone-500">Total Soal Bagian Ini: </span>
                              ...
                              <div id="input-cognitive">
                                  <label className="block text-xs font-bold text-stone-600 mb-2 uppercase">Dimensi Kognitif</label>
                                  
                                  <div className={`space-y-3 ${formErrors.cognitive ? 'p-2 border border-red-300 bg-red-50 rounded-lg' : ''}`}>
                                      {/* LOTS Group */}
                                      <div>
                                          <div className="bg-stone-100 px-2 py-1 text-[10px] font-bold text-stone-500 uppercase border-l-2 border-stone-400 mb-1">LOTS (Lower Order)</div>
                                          <div className="grid grid-cols-3 gap-2">
                                              {lotsLevels.map(level => (
                                                  <button key={level} onClick={() => handleCogCheck(level)}
                                                      className={`text-[10px] px-2 py-2 rounded-lg border transition-all text-center ${currentSection.cognitiveLevels[level] ? 'bg-emerald-700 text-white border-emerald-700' : 'bg-white text-stone-600 border-stone-300 hover:bg-stone-50'}`}>
                                      ...
                                      {/* HOTS Group */}
                                      <div>
                                          <div className="bg-stone-100 px-2 py-1 text-[10px] font-bold text-stone-500 uppercase border-l-2 border-amber-400 mb-1">HOTS (Higher Order)</div>
                                          <div className="grid grid-cols-3 gap-2">
                              ...
                              <button onClick={openCustomInstrModal} className="w-full bg-stone-800 hover:bg-stone-900 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 text-sm shadow-sm">
                  ...
                  <div className="lg:col-span-7 flex flex-col">
                      <div className="bg-white shadow-sm border border-stone-200 rounded-xl flex-1 flex flex-col overflow-hidden">
                          <div className="bg-stone-50 px-5 py-3 border-b border-stone-200 flex items-center gap-2">
                             <LayoutList size={16} className="text-stone-700" />
                             <h3 className="text-sm font-bold text-stone-700 uppercase">Struktur Paket Soal</h3>
                          </div>
                          
                          <div className="flex-1 p-5 space-y-3 bg-white min-h-[300px]">
                              {sections.map((sec, idx) => (
                                  <div key={sec.id} className="bg-white border border-stone-300 rounded-xl p-4 hover:border-emerald-400 transition-all shadow-sm group relative">
                                      {/* Delete Button - Absolute Top Right */}
                                      <button onClick={() => sec.id && removeSection(sec.id)} className="absolute top-2 right-2 text-stone-400 hover:text-red-500 transition-colors">
                                          <Trash2 size={16} />
                                      </button>
                                      ...
                                      <div className="mb-3 pr-6">
                                          <h4 className="font-bold text-sm text-stone-800 uppercase">{sec.questionType}</h4>
                                          {sec.questionType.includes('Pilihan Ganda') && (
                                            <span className="text-[10px] font-medium text-stone-500 bg-stone-100 px-1.5 py-0.5 rounded-sm inline-block mt-1">
                                      ...
                                      <div className="mt-2 text-[10px] text-stone-600 bg-amber-50 border border-amber-200 p-2 rounded-lg">
                                        <span className="font-bold">Custom:</span> {sec.customInstruction}
                                      </div>
                                      )}

                                      {/* Stats Grid */}
                                      <div className="grid grid-cols-2 gap-3 text-xs border-t border-dashed border-stone-200 pt-3">
                                          {/* Col 1: Total & Difficulty */}
                                          <div className="border-r border-stone-200 pr-2">
                                              <p className="text-stone-400 text-[10px] uppercase font-bold mb-1">Distribusi Soal</p>
                                              <div className="space-y-1">
                                                  <div className="flex justify-between items-center">
                                                      <span className="text-stone-600">Total</span>
                                                      <span className="font-bold text-stone-800">{sec.totalQuestions}</span>
                                                  </div>
                                                  {['Mudah', 'Sedang', 'Sulit'].map(diff => {
                                                      ...
                                                      return (
                                                          <div key={diff} className="flex justify-between items-center text-[10px]">
                                                              ...
                                                              <span className="font-medium bg-stone-100 px-1.5 rounded-sm text-stone-700">{sec.difficultyCounts[diff]}</span>
                                                          </div>
                                                      )
                                                  })}
                                              </div>
                                          ...
                                          {/* Col 2: Cognitive */}
                                          <div className="pl-1">
                                              <p className="text-stone-400 text-[10px] uppercase font-bold mb-1">Kognitif</p>
                              ...
                              {sections.length === 0 && (
                                  <div className="h-full flex flex-col items-center justify-center text-center py-12 border-2 border-dashed border-stone-200 rounded-xl bg-stone-50/50">
                                      <p className="text-stone-400 text-sm font-medium">Data struktur soal masih kosong.</p>
                                      <p className="text-stone-400 text-xs">Silakan tambahkan bagian soal melalui form di sebelah kiri.</p>
                                  </div>
                              )}
                          </div>
                          ...
                          <div className="p-5 bg-gray-50 border-t border-gray-200">
                              <button 
                                  ...
                                  className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-3 rounded-lg shadow-sm flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm uppercase tracking-wide"
                              >
                                  {loading ? <Loader2 className="animate-spin" /> : <Wand2 className="text-amber-300" />}
          ...
          {currentView === 'result' && (
              <div ref={resultRef} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  {loading ? (
                      <div className="flex flex-col items-center justify-center min-h-[400px] p-8 text-center bg-white rounded-xl border border-stone-200 shadow-sm">
                          <Loader2 className="w-12 h-12 text-emerald-700 animate-spin mb-6" />
                          <h3 className="text-xl font-bold text-stone-800 mb-2 uppercase tracking-wide">{loadingStatus || 'SEDANG MEMPROSES PERMINTAAN...'}</h3>
                          <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mt-8 max-w-lg text-left shadow-sm rounded-r-lg">
                              <div className="flex">
                                  <div className="flex-shrink-0">
                                      <AlertTriangle className="h-5 w-5 text-amber-500" aria-hidden="true" />
                                  </div>
                                  <div className="ml-3">
                                      <p className="text-sm text-amber-800 font-bold uppercase tracking-wide">
                                          PERINGATAN PENTING
                                      </p>
                      ...
                      <div className="bg-white rounded-sm shadow-md border border-gray-300 overflow-hidden relative min-h-[800px] flex flex-col">
                          {/* Toolbar Sticky */}
                          <div className="bg-stone-100 p-3 border-b border-stone-300 flex justify-between items-center sticky top-0 z-20 shadow-sm">
                              <div className="flex gap-1 items-center">
                                  {['soal', 'kunci', 'kisi'].map(t => (
                                      <button key={t} onClick={() => setActiveTab(t)}
                                          className={`px-4 py-1.5 text-xs font-bold uppercase tracking-wide border rounded-sm transition-colors ${activeTab === t ? 'bg-white border-gray-300 text-emerald-800 shadow-sm' : 'bg-transparent border-transparent text-stone-500 hover:bg-stone-200'}`}>
                              ...
                              <div className="flex gap-2">
                                  ...
                                  {failedImageCount > 0 && (
                                      <button 
                                        ...
                                        className="bg-amber-500 hover:bg-amber-600 text-emerald-900 px-4 py-1.5 rounded-lg shadow-sm flex items-center gap-2 text-xs font-bold transition-colors uppercase disabled:opacity-50 disabled:cursor-not-allowed"
                                      >
                                  ...
                                  <button onClick={handleExportWord} className="bg-emerald-700 hover:bg-emerald-800 text-white px-4 py-1.5 rounded-lg shadow-sm flex items-center gap-2 text-xs font-bold transition-colors uppercase">
                          ...
                          <div className="p-10 bg-stone-500/10 min-h-[700px] overflow-auto flex-1">
                                                  ...
                                                  <div key={qIdx} className={`flex gap-3 text-sm font-serif leading-relaxed text-black transition-opacity ${isRegeneratingText ? 'opacity-50' : 'opacity-100'}`}>
                                                      ...
                                                      <div className="w-full">
                                                          ...
                                                          {(q.image || q.image_prompt || isRegenerating || isEditingPrompt) && (
                                                              <div className="flex flex-col md:flex-row items-start gap-4 mb-3 border border-stone-200 p-2 rounded-lg bg-stone-50/50">
                                                                      ...
                                                                      {isEditingPrompt ? (
                                                                          ...
                                                                          <div className="space-y-2 animate-in fade-in slide-in-from-left-2 duration-200">
                                                                              ...
                                                                              <textarea 
                                                                                  ...
                                                                                  className="w-full text-xs p-2 border border-emerald-300 bg-white text-stone-900 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none h-20 font-sans"
                                                                              ...
                                                                              <div className="flex gap-2 justify-end">
                                                                                  ...
                                                                                  <button 
                                                                                      ...
                                                                                      className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold font-sans uppercase flex items-center gap-1.5 shadow-sm"
                                                                                  >
                                                                          ...
                                                                          <div className="flex flex-col gap-2 h-full justify-center">
                                                                              ...
                                                                              <div className="flex flex-wrap gap-2">
                                                                                  <button 
                                                                                    ...
                                                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg text-amber-800 text-xs font-bold font-sans uppercase tracking-wide transition-colors disabled:opacity-50"
                                                                                  >
                                                                                      <Edit3 size={12} />
                                                                                      Edit Prompt
                                                                                  </button>
                                                                                  ...
                                                                                  <button 
                                                                                    ...
                                                                                    disabled={isRegenerating || isRegeneratingText}
                                                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-emerald-700 text-xs font-bold font-sans uppercase tracking-wide transition-colors disabled:opacity-50 group"
                                                                                  >
                                                                                  ...
                                                                                  <button 
                                                                                    ...
                                                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg text-red-700 text-xs font-bold font-sans uppercase tracking-wide transition-colors disabled:opacity-50"
                                                                                  >
                                                                                      <Trash2 size={12} />
                                                                                      Hapus
                                                                                  </button>
                                                          ...
                                                          <button
                                                              ...
                                                              className="flex items-center gap-1.5 px-3 py-1 text-[10px] font-sans font-bold text-stone-500 hover:text-emerald-700 bg-stone-100 hover:bg-emerald-50 border border-stone-200 hover:border-emerald-200 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                                          >
                                          ...
                                          {generatedData.ujian.map((part, idx) => (
                                            <div key={idx}>
                                                ...
                                                <table className="w-full text-xs font-serif border-collapse border border-black text-black">
                                                    <thead>
                                                        <tr className="bg-stone-100 text-black"><th className="border border-black p-1 w-10">No</th><th className="border border-black p-1">Jawaban</th></tr>
                                                    </thead>
                                                    <tbody>
                                                        {(part.soal || []).map(q => (
          ...
          {currentView === 'quiz' && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
                  ...
                  {!isPlayMode && (
                      <div className="mb-6 flex justify-between items-center border-stone-300 pb-4">
                         <div>
                            <h2 className="text-2xl font-bold text-stone-800 uppercase tracking-tight flex items-center gap-2">
                  ...
                  {quizMode === 'edit' && generatedData && (
                      <div className="flex-1 flex flex-col">
                          <div className="flex-1 bg-white border border-stone-300 rounded-xl shadow-sm overflow-hidden flex flex-col">
                              <div className="bg-stone-50 px-6 py-3 border-b border-stone-200 flex justify-between items-center">
                                  <h3 className="font-bold text-stone-700 text-sm uppercase">Daftar Soal Siap Kuis</h3>
                                  <div className="text-xs text-stone-500 font-medium">Total Soal: {generatedData.ujian.reduce((acc, part) => acc + (part.soal ? part.soal.length : 0), 0)}</div>
                              </div>
                              
                              <div className="flex-1 overflow-y-auto p-6 bg-stone-100 space-y-4">
                                  {generatedData.ujian.map((part, idx) => (
                                      <div key={idx} className="space-y-4">
                                          <div className="text-stone-500 uppercase tracking-wider pl-1">{part.judul_bagian}</div>
                                          {(part.soal || []).map((q, qIdx) => {
                                              const isRegenerating = regeneratingIds[`${idx}-${qIdx}`];
                                              const isRegeneratingText = regeneratingQuestionIds[`${idx}-${qIdx}`];

                                              return (
                                                  <div key={qIdx} className={`bg-white p-4 rounded-xl border border-stone-200 shadow-sm flex gap-4 ${isRegeneratingText ? 'opacity-50' : 'opacity-100'}`}>
                                                      <div className="font-bold text-emerald-800 text-lg w-8 text-center bg-emerald-50 rounded-lg pt-1">{q.no}</div>
                                                      <div className="flex-1">
                                                          ...
                                                          {(q.image || q.image_prompt) && (
                                                              <div className="flex items-start gap-4 mb-3 p-3 bg-stone-50 border border-stone-200 rounded-lg">
                                                                  <div className="relative border border-stone-300 p-1 bg-white min-w-[100px] min-h-[80px] flex items-center justify-center">
                                                                      {isRegenerating ? <Loader2 className="animate-spin text-emerald-500" /> : 
                                                                       q.image ? <img src={q.image} alt="Ilustrasi" className="max-w-[150px] h-auto" /> : <span className="text-xs text-stone-400">Tidak ada gambar</span>}
                                                                  </div>
                                                          ...
                                                          <div className="mt-2 flex">
                                                              <button
                                                                  onClick={() => handleRegenerateQuestion(idx, qIdx)}
                                                                  disabled={isRegeneratingText || isRegenerating}
                                                                  className="flex items-center gap-1.5 px-3 py-1.5 text-stone-500 hover:text-emerald-700 bg-stone-100 hover:bg-emerald-50 border border-stone-200 hover:border-emerald-200 rounded-lg transition-colors disabled:opacity-50"
                                                              >
                              ...
                              <div className="p-4 bg-white border-t border-gray-200">
                                  <button 
                                    onClick={startQuiz}
                                    className="w-full bg-blue-700 hover:bg-blue-800 text-white font-bold py-4 rounded-sm shadow-md text-lg uppercase tracking-wider flex items-center justify-center gap-3 transition-transform hover:-translate-y-0.5"
                                  >
                  ...
                  {quizMode === 'play' && quizState.flatQuestions.length > 0 && (
                      <div className="flex-1 bg-stone-900 overflow-hidden flex flex-col relative text-white p-8">
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center max-w-5xl mx-auto w-full pt-16">
                              ...
                              {quizState.flatQuestions[quizState.currentIdx].type!.includes('Pilihan Ganda') ? (
                                  ...
                                  <div className="w-full text-center space-y-6">
                                      <div className="text-stone-400 italic">Soal ini bukan Pilihan Ganda. Diskusikan jawabannya bersama kelas!</div>
                                      {!quizState.showAnswer ? (
                                          <button 
                                            onClick={() => setQuizState(prev => ({...prev, showAnswer: true, isAnswered: true}))}
                                            className="bg-amber-500 hover:bg-amber-600 text-black font-bold py-3 px-8 rounded-full text-lg shadow-lg"
                                          >
                                              Lihat Kunci Jawaban
                                          </button>
                                      ) : (
                  ...
                  {quizMode === 'score' && (
                      <div className="flex-1 flex flex-col items-center justify-center bg-stone-900 text-white p-8 text-center animate-in zoom-in duration-500 relative">
                          ...
                          <Trophy className="w-24 h-24 text-amber-400 mb-6 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)]" />
        ...
        {/* --- FOOTER COPYRIGHT --- */}
        {showSidebar && (
        <footer className={`fixed bottom-0 right-0 w-full bg-stone-200 border-t border-stone-300 py-2 z-40 ${currentView !== 'welcome' ? 'md:w-[calc(100%-14rem)]' : ''}`}>
            <div className="max-w-7xl mx-auto px-6 text-center text-[10px] text-stone-500 font-bold uppercase tracking-wider">
                © 2025 RUANG ASET ID. Hak Cipta Dilindungi Undang-Undang. | Version 1.2 (Stable)
            </div>
        </footer>
        )}
      </div>
